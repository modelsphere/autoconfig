...
# lint + template + helm install → controller 起来 → 一个 RouterBinding 调谐 → uninstall(CRD 保留)。
...
cleanup(){ [ "$KEEP" = 1 ] && { echo "(--keep)"; return; }
  ...
  kubectl delete crd routerbindings.routing.4pd.io --wait=false 2>/dev/null
}
trap cleanup EXIT
...
kubectl get crd routerbindings.routing.4pd.io >/dev/null 2>&1 && ok "CRD 已安装" || bad "CRD 未安装"

say "mock 后端 + CART + RouterBinding(helm 装的 controller 调谐)"
...
waiteq 2 "rb.status.backends" kubectl -n "$TNS" get rb demo -o jsonpath='{.status.backends}'
waiteq true "rb.status.ready" kubectl -n "$TNS" get rb demo -o jsonpath='{.status.ready}'
...
kubectl get crd routerbindings.routing.4pd.io >/dev/null 2>&1 && ok "CRD 卸载后仍保留(keep)" || bad "CRD 被误删"
