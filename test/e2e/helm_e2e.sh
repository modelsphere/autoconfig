...
cleanup(){ [ "$KEEP" = 1 ] && { echo "(--keep)"; return; }
  ...
  kubectl delete crd modelroutes.routing.modelsphere.dev --wait=false 2>/dev/null
}
trap cleanup EXIT
...
kubectl get crd modelroutes.routing.modelsphere.dev >/dev/null 2>&1 && ok "CRD 已安装" || bad "CRD 未安装"
...
kubectl get crd modelroutes.routing.modelsphere.dev >/dev/null 2>&1 && ok "CRD 卸载后仍保留(keep)" || bad "CRD 被误删"
