#!/usr/bin/env bash
# 验证 0.3.13 三改动,用现成的真 vllm(e2e-test/vllm-mock-vllm-svc,opt-125m,跨 ns 发现,不动 GPU):
...
# 复用 kimi ns 里已装的 openresty/cart/monitor(controller 已 0.3.13)。用法:bash verify_v12_vllm.sh
set -uo pipefail
NS=${NS:-kimi}
